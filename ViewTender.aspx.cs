...
public partial class pages_Tender : System.Web.UI.Page
{
    ...
    private void BindGridView(string searchQuery = "")
    {
        ...
        {
            string query = "SELECT DocsID, No, Title, Date, FilePath FROM Docs WHERE Type = 'Tender' ORDER BY Date DESC";
            if (!string.IsNullOrEmpty(searchQuery))
            {
                query = "SELECT DocsID, No, Title, Date, FilePath FROM Docs WHERE Type = 'Tender' AND Title LIKE @SearchQuery ORDER BY Date DESC";
            }
