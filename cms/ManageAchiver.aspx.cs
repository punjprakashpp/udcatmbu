...
public partial class ManageAchiver : System.Web.UI.Page
{
    ...
    private void LoadSessionDropdown()
    {
        ...
        {
            string query = "SELECT DISTINCT Session FROM Achiver ORDER BY Session";
    ...
    protected void btnSearch_Click(object sender, EventArgs e)
    {
        ...
        string query = "SELECT StudentID, Session, FirstName, MidName, LastName " +
                       "FROM Achiver WHERE Session = @Session AND FirstName LIKE @SearchParam";
    ...
    private void BindGridData(DataTable studentTable)
    {
        ...
        if (studentTable.Rows.Count > 0)
        {
            ...
            ShowMessage(String.Format("{0} Achiver(s) Found.", studentTable.Rows.Count), false);
        }
        else
        {
            MultiView1.ActiveViewIndex = -1;
    ...
    protected void GridView1_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (e.CommandName == "Manage")
        {
            ...
            {
                ShowMessage("Invalid Achiver ID.", true);
            }
        }
    }
    ...
    {
        string query = "SELECT * FROM Achiver WHERE StudentID = @StudentID";
            ...
            try
            {
                ...
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    if (reader.Read())
                    {
                        ...
                        txtAchivement.Text = reader["Achivement"].ToString();
                    ...
                    {
                        ShowMessage("Achiver details not found.", true);
                    }
                }
            }
            catch (Exception ex)
        ...
        using (SqlConnection con = new SqlConnection(connectionString))
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                ...
                try
                {
                    ...
                    if (rowsAffected > 0)
                    {
                        ShowMessage("Achiver details updated successfully.", false);
                    }
                    else
                    {
    ...
    protected void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        {
            string query = "SELECT FilePath FROM Achiver WHERE StudentID = @StudentID";
        ...
        string deleteQuery = "DELETE FROM Achiver WHERE StudentID = @StudentID";
        ...
        using (SqlCommand cmd = new SqlCommand(deleteQuery, con))
        {
            ...
            try
            {
                con.Open();
                cmd.ExecuteNonQuery();
                ShowMessage("Achiver removed successfully.", false);
            }
            catch (Exception ex)
            {
                ShowMessage("Error: " + ex.Message, true);
