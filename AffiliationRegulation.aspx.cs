...
public partial class pages_AffiliationRegulation : System.Web.UI.Page
{
    ...
    private void BindGridView(string searchQuery = "")
    {
        ...
        {
            string query = "SELECT DocsID, Title, Date, FilePath FROM Docs WHERE Type = 'AffReg' ORDER BY Date DESC";
            if (!string.IsNullOrEmpty(searchQuery))
            {
                query = "SELECT DocsID, Title, Date, FilePath FROM Docs WHERE Type = 'AffReg' AND Title LIKE @SearchQuery ORDER BY Date DESC";
            }
