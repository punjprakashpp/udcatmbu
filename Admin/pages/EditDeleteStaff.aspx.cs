...
public partial class Admin_pages_EditFaculty : System.Web.UI.Page
{
    ...
    private void PopulateFacultyDropdown()
    {
        ...
        using (SqlConnection conn = new SqlConnection(connStr))
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                ...
                ddlFaculties.DataValueField = "StaffId";
    ...
    private void PopulateFacultyDetails(int facultyId)
    {
        ...
        using (SqlConnection conn = new SqlConnection(connStr))
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, conn))
            {
                cmd.Parameters.AddWithValue("@StaffId", facultyId);
                conn.Open();
                SqlDataReader reader = cmd.ExecuteReader();
                if (reader.Read())
                {
    ...
    protected void btnUpdate_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(ddlFaculties.SelectedValue))
        {
            ...
            using (SqlConnection con = new SqlConnection(connStr))
            {
                ...
                string selectImagePathQuery = "SELECT ImagePath FROM Staff WHERE StaffId = @StaffId";
                using (SqlCommand selectImagePathCmd = new SqlCommand(selectImagePathQuery, con))
                {
                    selectImagePathCmd.Parameters.AddWithValue("@StaffId", facultyId);
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    string query = "UPDATE Staff SET Name = @Name, Qualification = @Qualification, Position = @Position, Phone = @Phone, Email = @Email, ImagePath = @ImagePath WHERE StaffId = @StaffId";
                    ...
                    using (SqlCommand cmd = new SqlCommand(query, conn))
                    {
                        ...
                        cmd.Parameters.AddWithValue("@StaffId", ddlFaculties.SelectedValue);
                        conn.Open();
                        cmd.ExecuteNonQuery();
        ...
        if (!string.IsNullOrEmpty(ddlFaculties.SelectedValue))
        {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    string deleteFacultyQuery = "DELETE FROM Staff WHERE StaffId = @StaffId";
                    using (SqlCommand deleteFacultyCmd = new SqlCommand(deleteFacultyQuery, conn))
                    {
                        deleteFacultyCmd.Parameters.AddWithValue("@StaffId", facultyId);
